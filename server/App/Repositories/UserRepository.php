...
class UserRepository
{
    ...
    public function create(array $data)
    {
        ...
        $userEntity->setEmail($data["email"] ?? "")->setDisplayName($data["displayName"] ?? "")->setPassword(
            $data["password"] ?? ""
        )
            ->setAvatar($this->config->get('user')['default']['avatar'])->setAvatarPublicId(
                $this->config->get('user')['default']['avatarPublicId']
            )
    ...
    {
        $userEntity = new UserEntity();
        $userEntity->setEmail($data['email'] ?? $originalData['email'])->setDisplayName(
            $data['displayName'] ?? $originalData['displayName']
        )
            ->setAvatar($data['secureUrl'] ?? $originalData['avatar'])->setAvatarPublicId(
                $data['publicId'] ?? $originalData['avatarPublicId']
            )
            ->setAccessToken($data['accessToken'] ?? $originalData['accessToken'])->setRefreshToken(
                $data['refreshToken'] ?? $originalData['refreshToken']
            )
            ->build();
